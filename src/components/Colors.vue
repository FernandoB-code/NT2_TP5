<template>
  <div>
    ...
    <div id="container">
      <div class="square" :style="{'background-color':this.colors[0]}" @click="comparator(0)"></div>
      <div class="square" :style="{'background-color':this.colors[1]}" @click="comparator(1)"></div>
      <div class="square" :style="{'background-color':this.colors[2]}" @click="comparator(2)"></div>
      <div class="square" :style="{'background-color':this.colors[3]}" @click="comparator(3)" v-if="isHard"></div>
      <div class="square" :style="{'background-color':this.colors[4]}" @click="comparator(4)" v-if="isHard"></div>
      <div class="square" :style="{'background-color':this.colors[5]}" @click="comparator(5)" v-if="isHard"></div>
    </div>
  ...
  methods: {
    ...
    randomInt(){
     ...
     comparator(index){      
      ...
      if(this.colors[index] == this.pickedColor){
        this.setAllColorsTo(this.colors[index])
        this.mensage = 'You Picked Right!'
        this.restartButton = 'Play Again!'
        this.backgroundColor = this.pickedColor
      }
      else {
        document.querySelectorAll('.square')[index].style.backgroundColor = '#232323'
        this.mensage = 'Try Again!'
      }
    },
